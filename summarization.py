...
def main(args):
    ...
    if "abisee/cnn_dailymail" in args.dataset:
        dataset = load_dataset(args.dataset, "3.0.0", split=args.split)
    else: 
    ...
    with torch.no_grad():
        for i, input_item in enumerate(tqdm(test_dataloader, desc="Predicting...")): 
            ...
            elif "abisee/cnn_dailymail" in args.dataset:
                input_text = input_item["article"]
                gold = input_item["highlights"]
                model_inputs = tokenizer(input_text, max_length=2000, padding=True, truncation=True, return_tensors='pt').to("cuda")
                
            elif "EdinburghNLP/xsum" in args.dataset:
                input_text = input_item["document"]
                gold = input_item["summary"] 
                model_inputs = tokenizer(input_text, max_length=2000, return_tensors='pt')
